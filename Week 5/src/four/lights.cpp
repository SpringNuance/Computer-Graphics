...
void PointLight::getIncidentIllumination(const Vec3f& p,
	...
	dir_to_light = (this->position_ - p).normalized();
	incident_intensity = this->intensity_ * 1.0f/(float)(this->quadratic_attenuation_ * distance * distance + this->linear_attenuation_ * distance + this->constant_attenuation_);
	distance = (this->position_ - p).length();
}
