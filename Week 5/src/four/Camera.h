...
public:
	...
	static inline FW::Vec2f normalizedImageCoordinateFromPixelCoordinate(const FW::Vec2f& pixel, const FW::Vec2i& imageSize) {
		...
		return FW::Vec2f((float) pixel.x / (0.5f *(float)imageSize.x) - 1, - ((float)pixel.y / (0.5f * (float)imageSize.y) - 1));
	}
...
public:
	...
	virtual Ray generateRay(const FW::Vec2f& point) {
		...
		// coordinate on the camera plane normalized from 
		FW::Vec3f camOrigin = center + 0.5f * point.x * size * horizontal + 0.5f * point.y * size * up;
		return Ray(camOrigin, direction); // Since direction is constant in orthographic camera
	}
...
public:
	...
	{	
		...
		float distance = (float) 1 / (float) FW::tan(0.5f * fov_angle);
		// Aspect ratio is 1 since the viewport is a square
		FW::Vec3f r = point.x * horizontal + point.y * up + distance * direction; 
		r.normalize();
		return Ray(center, r);
