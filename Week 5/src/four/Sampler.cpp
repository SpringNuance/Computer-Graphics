...
Vec2f RegularSampler::getSamplePosition(int n) {
	...
	Vec2f sampler;
	if (m_dim != 0 ) {
		sampler.x = ((float)(n % m_dim) - 0.5f)/(float) m_dim;
		sampler.y = ((float)(n / m_dim) - 0.5f)/(float) m_dim;
	}
	return sampler;
}
...
Vec2f JitteredSampler::getSamplePosition(int n) {
	...
	Vec2f sampler;
	if (m_dim != 0) {
		float rand_x = (float)(rand() % RAND_MAX) / (float)m_dim;
		float rand_y = (float)(rand() % RAND_MAX) / (float)m_dim;
		sampler.x = (float)(n % m_dim) / (float) (m_dim)+rand_x;
		sampler.y = (float)(n / m_dim) / (float)(m_dim)+rand_y;
	}
	return sampler;
}
