	...
	for (int j = 0; j < args.height; ++j) {
		...
		for (int i = 0; i < args.width; ++i) {
			...
			for (int n = 0; n < args.num_samples; ++n) {
				...
				image->setVec4f(Vec2i(i,j), Vec4f(sample_color/args.num_samples, 1) + Vec4f(image->getVec4f(Vec2f(i, j))));
				...
				if (args.display_uv)
					sample_color = Vec3f((float) i/ ((float)args.width - 1), (float)j/ ((float)args.height - 1), 1);
				...
				if (depth_image) {
					...
					float t_range = args.depth_max - args.depth_min;
					// Clamping t value
					if (hit.t < args.depth_min) hit.t = args.depth_min;
					if (hit.t > args.depth_max) hit.t = args.depth_max;
					float f = (float) - (hit.t - args.depth_min) / (float) t_range + 1;
					/*float f = 0.0f;
					if (hit.t >= args.depth_min && hit.t <= args.depth_max)
						f = 1 - (float)(hit.t - args.depth_min) / (float)(args.depth_max - args.depth_min);*/
