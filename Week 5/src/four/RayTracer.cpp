...
Vec3f mirrorDirection(const Vec3f& normal, const Vec3f& incoming) {
	...
	return (incoming - 2 * dot(incoming, normal) * normal).normalized();
}
...
Vec3f RayTracer::traceRay(Ray& ray, float tmin, int bounces, float refr_index, Hit& hit, FW::Vec3f debug_color) const {
	...
	for (int i = 0; i < scene_.getNumLights(); i++) {
		Vec3f directionToLight, incidentIntensity;
		float distanceToLight;
		Light * lightPtr = scene_.getLight(i);
		lightPtr->getIncidentIllumination(point, directionToLight, incidentIntensity, distanceToLight);
		Ray ray2 = Ray(ray.pointAtParameter(hit.t) - ray.direction * EPSILON, -directionToLight);
		Hit hit2(distanceToLight);
		if (args_.shadows) {
			if (!scene_.getGroup()->intersect(ray2, hit2, EPSILON)) 
			answer += m->shade(ray, hit, directionToLight, incidentIntensity, args_.shade_back);
		} else answer += m->shade(ray, hit, directionToLight, incidentIntensity, args_.shade_back);
	...
	if (bounces >= 1) {
		...
		if (m->reflective_color(point).length() > 0.0f) {
			...
			Vec3f dirFromMirror = mirrorDirection(normal, ray.direction);
			Ray reflectionRay = Ray(ray.pointAtParameter(hit.t) - EPSILON * ray.direction, dirFromMirror);
			Hit reflectionHit = Hit();
			answer += hit.material->reflective_color(point) * traceRay(reflectionRay, 0, bounces - 1, refr_index, reflectionHit, debug_color);
