		...
		bool shade_back) const
{
	...
	// Diffuse Light
	Vec3f diffuse(0.f), specular(0.f);
	Vec3f normal = hit.normal;

	float d = -dot(dir_to_light, normal);
	if (d < 0) d = 0.f; // Clamping
	
	// D_i = c_light_i ∗ clamp(Li · N) ∗ c_object_diffuse
	diffuse = incident_intensity * d * hit.material->diffuse_color(ray.pointAtParameter(hit.t));
		
	Vec3f ray_direction = ray.direction.normalized();

	// Specular Light
	Vec3f r_i = (ray_direction - 2.0f * dot(ray_direction, normal) * normal).normalized(); // Ideal reflection vector
	float s = -dot(dir_to_light, r_i);
	if (s < 0) s = 0.f;
	
	// S_i = clight_i ∗ k_s ∗ (clamp(dot(v, ri)))^q
	specular = incident_intensity * specular_color() * powf(s, this->exponent_);
		
	// Phong shading formula: I = Ambient + SUM_i(Diffuse_i + Specular_i)
	// Ambient lighting is added in raytracer.cpp later. Now only adds up Diffuse and Specular
	Vec3f answer = diffuse + specular;
	return answer;
}
